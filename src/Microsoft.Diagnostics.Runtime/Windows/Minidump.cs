...
namespace Microsoft.Diagnostics.Runtime.Windows
{
    internal sealed class Minidump : IDisposable
    {
        ...
        /// etc.) when the dump contains a <c>MiscInfoStream</c>. Otherwise zero.</summary>
        public MinidumpMiscInfo MiscInfo { get; private set; }

        /// <summary>Whether the dump contained a <c>MiscInfoStream</c>.</summary>
        public bool HasMiscInfo { get; private set; }
        ...
        public Minidump(string displayName, Stream stream, CacheOptions cacheOptions, bool leaveOpen, DataTargetLimits? limits = null)
        {
            ...
            if (miscStream != -1)
            {
                ...
                MiscInfo = Read<MinidumpMiscInfo>(stream, _directories[miscStream].Rva);
                HasMiscInfo = true;
                ...
                if (MiscInfo.SizeOfInfo >= MinSizeForBuildString
                    && (MiscInfo.Flags1 & MinidumpMiscInfo.MINIDUMP_MISC4_BUILDSTRING) != 0)
                {
