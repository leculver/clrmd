...
namespace Microsoft.Diagnostics.Runtime
{
    internal sealed class MinidumpReader : IDataReader, IDisposable, IThreadReader, IDumpInfoProvider, IDumpFileMemorySource, IThreadInfoReader, IProcessInfoProvider, IMemoryRegionReader
    {
        ...
        public ProcessInfo GetProcessInfo()
        {
            ...
            if (_minidump.HasMiscInfo
                && (_minidump.MiscInfo.Flags1 & MinidumpMiscInfo.MINIDUMP_MISC1_PROCESS_TIMES) != 0)
            {
                createTime = _minidump.MiscInfo.ProcessCreateTime != 0
                    ? DateTimeOffset.FromUnixTimeSeconds(_minidump.MiscInfo.ProcessCreateTime).UtcDateTime
                    : (DateTime?)null;
                userTime = TimeSpan.FromSeconds(_minidump.MiscInfo.ProcessUserTime);
                kernelTime = TimeSpan.FromSeconds(_minidump.MiscInfo.ProcessKernelTime);
            }
